...
def process_for_answers(tokenizer, row):
    a = tokenizer.encode(row['answer'], max_length=512, add_special_tokens=False)
    ...
    if a_len + 2 > 512:
        answer_trunc = a[:512-2]
    else:
        answer_trunc = a

    combined = answer_trunc + [tokenizer.sep_token_id]
...
def main(**args):
    ...
    train_df['all'] = train_df.apply(lambda x: process_for_answers(tokenizer, x), axis=1)
    ...
    if args['do_wandb']:
        wandb.init(project=args['project'], tags=['answers'])
        wandb.watch(model)
...
def _run_get_preds(df, ckpt_dir, fold_n, params):
    ...
    with open('.tmp/answers_preds.pickle', 'wb') as f:
        pickle.dump(all_preds, f)

# example: python train_bert_on_answers.py --do_apex --do_wandb --maxlen 256 --bs 8 --dp 0.1 --fold 0 --out_dir test
# trained model will be saved to model/test_fold_0
# python train_bert_on_answers.py --do_apex --do_wandb --maxlen 256 --bs 8 --dp 0.1 --out_dir test
if __name__ == '__main__':
    ...
    parser.add_argument("--out_dir", default="outputs/bert_answers", type=str)
